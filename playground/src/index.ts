...
const config: MapperConfig<typeof user> = {
	name: {
		...
		options: { classification: 'pii', key: () => 'fullName' } as AnonymizePluginPropertyOptions,
	},
	birthday: {
		value: (data) => formatISO(data.birthday, { representation: 'date' }),
		options: { classification: 'pii', anonymize: 'redact' } as AnonymizePluginPropertyOptions,
	},
